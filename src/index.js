...
import PixabayApiServise from './fetch';
...
function onMessage(hits, totalHits, perPage) {
...
async function appendMarkup() {
  try {
    ...
    refs.galleryImages.insertAdjacentHTML('beforeend', renderMarkup(hits));
    perPageCounter();
    onMessage(hits, totalHits, perPage);
  } catch (error) {
